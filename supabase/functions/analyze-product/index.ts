...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Analyzing product image with Google Gemini...");
        ...
        contents: [
          {
            parts: [
              {
                text: `あなたは日本の食品パッケージを読み取り、韓国語に翻訳する専門家です。

**重要な指示:**
1. 画像から日本語のテキストを正確に読み取ってください（商品名、原材料、説明文など）
2. パッケージに書かれた情報を元に商品を分析してください
3. 各商品について以下の情報を提供してください:
   - 商品名（日本語）
   - 商品名の読み方（ひらがな）
   - 商品名の韓国語翻訳
   - カテゴリー（例：お菓子、飲料、調味料、即席食品など）
   - 主な原材料（パッケージから読み取れる場合）
   - 味や特徴の説明
   - 似ている韓国の食品（あれば）
   - 食べ方や調理方法（あれば）
  ...
  "items": [
    {
      ...
      "ingredients": ["재료1", "재료2"],
      "taste": "맛과 특징 설명",
      "similarKorean": "비슷한 한국 식품",
      "howToEat": "먹는 방법/조리법",
      "searchQuery": "Yahoo Japanショッピング検索用クエリ"
    }
  ]
}

複数の商品が写っている場合は、すべて識別して個別に分析してください。
パッケージから読み取れない情報がある場合は、空文字列または空配列を返してください。

이 일본 식품 사진을 분석해주세요. 상품명을 읽고, 한국어로 번역하고, 상세 정보를 제공해주세요.`
              },
              {
                inlineData: {
                  mimeType: mimeType,
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ 
          error: "상품 분석 결과를 파싱할 수 없습니다. 다시 시도해주세요."
        }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    console.log("Product analysis complete:", productData.items?.length || 0, "items found");
